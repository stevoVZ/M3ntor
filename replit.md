...
- **Smart Type Suggestion**: `getItemHint()` returns `suggestedType` and `typeReason`; FabActionSheet shows all 4 types in a 2x2 grid with the AI-recommended type pre-highlighted; falls back to local `inferType()` NLP when API fails; all user-facing AI labels use "M3NTOR" branding
