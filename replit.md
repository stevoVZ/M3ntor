...
- **Journey Management**: `enrollJourney`, `unenrollJourney`, `reEnrollJourney(journeyId, reset)`, `advanceJourneyDay` — unenrolling pauses the journey (preserves progress), re-enrolling offers resume or reset. Day/week advancement auto-completes the journey when the final week is done.
- **Soft Delete / Trash**: `removeItem` does a soft delete (sets `deleted_at`, moves to `deletedItems` array). `restoreItem` recovers from trash. `permanentlyDeleteItem` hard-deletes. Plan tab shows a collapsible "Recently Deleted" section with restore/delete options.
...
- **Design System**: `constants/theme.ts` exports `T` (colors/tokens — indigo `#4F46E5` brand), `S` (spacing), `F` (font sizes), `R` (border radii), `shadow` (platform shadows)
...
- **Client-side AI module**: `lib/ai.ts` exports `aiAssist()`, `getItemHint()`, `generateProjectTasks()`, `expandProjectPhase()`, `generateSubtasks()`, `generateGoal()` — all accept optional `country` param for region-aware responses
...
- **Phase-Aware Expansion**: When "More tasks" is tapped on a project with existing steps, an inline phase picker appears with two options: "Add new phases" (generates missing major phases) or "Expand a step" (pick an existing step to break down into detailed sub-steps inserted after it). `expandProjectPhase()` generates tasks specific to the chosen phase, aware of sibling phases to avoid duplication. New steps are re-ordered to appear right after the expanded step.
